...
def gen_ascon_key(passcode:bytes) -> bytes:
    assert isinstance(passcode, bytes)
    hkey = ascon.hash(passcode, variant="Ascon-Xof", hashlength=16)
    return hkey
        ...
        for elem2 in elem1:
            for elem3 in elem2:
                for elem4 in elem3:
                    k += elem4
    k = str(k)
    return gen_ascon_key(k.encode('utf-8'))
...
def cypherMessage( msg, resp, userIdPoint, deviceIdPoint):
    ...
    non = 1
    hnon = ascon.hash(str(non).encode('utf-8'), variant="Ascon-Xof", hashlength=16)
    print(f"Key: {key}")
    print(type(key))
    print(len(key))

    encMessage = ascon.encrypt(key,hnon , ("associateddata").encode('utf-8'), msg, variant="Ascon-128")
